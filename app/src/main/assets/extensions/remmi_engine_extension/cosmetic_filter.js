...
(function () {
  ...
  const STYLE_ID = 'remmi-cosmetic-styles';
  const MAX_PENDING_BATCH = 150;
  const SEEN_CLASSES = new Set();
  const SEEN_IDS = new Set();
  const INJECTED_SELECTORS = new Set();
  const PENDING_INJECTIONS = [];
  ...
  function getOrCreateStyleSheet() {
    ...
    if (!existing) {
      ...
      existing.setAttribute('data-remmi-cosmetic', '1');
      try {
  ...
  function flushPendingInjections() {
    batchInjectTimer = null;
    if (PENDING_INJECTIONS.length === 0) return;

    const styleEl = getOrCreateStyleSheet();
    if (!styleEl) {
      if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', flushPendingInjections, { once: true });
      } else {
        batchInjectTimer = setTimeout(flushPendingInjections, 50);
      }
      return;
    }
  ...
  function injectSelectors(selectors) {
    ...
    if (!batchInjectTimer) {
      if (typeof requestIdleCallback === 'function') {
        batchInjectTimer = requestIdleCallback(flushPendingInjections, { timeout: 100 });
      } else if (typeof requestAnimationFrame === 'function') {
        batchInjectTimer = requestAnimationFrame(flushPendingInjections);
      } else {
        batchInjectTimer = setTimeout(flushPendingInjections, 20);
      }
    }
  }
  ...
  function cleanupPageState() {
    ...
    if (batchInjectTimer) {
      if (typeof cancelIdleCallback === 'function') {
        try { cancelIdleCallback(batchInjectTimer); } catch (_e) {}
      }
      if (typeof cancelAnimationFrame === 'function') {
        try { cancelAnimationFrame(batchInjectTimer); } catch (_e) {}
      }
      clearTimeout(batchInjectTimer);
      batchInjectTimer = null;
    }
    PENDING_INJECTIONS.length = 0;
    pendingClassesSet.clear();
    pendingIdsSet.clear();
    SEEN_CLASSES.clear();
    SEEN_IDS.clear();
