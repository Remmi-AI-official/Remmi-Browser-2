  ...
  try {
    ...
    port.onMessage.addListener((msg) => {
      ...
      if (msg.type === "CLEAR_CACHE" || msg.type === "RULES_UPDATED") {
        ...
        if (scriptCode) {
          const numTabId = (typeof rawTabId === "number") ? rawTabId : (parseInt(rawTabId, 10) || null);
          const executeOnTab = (targetId) => {
            const params = { code: scriptCode, runAt: "document_end" };
            if (targetId !== null && !isNaN(targetId)) {
              return browser.tabs.executeScript(targetId, params);
            } else {
              return browser.tabs.executeScript(params);
            }
          };

          if (numTabId !== null && !isNaN(numTabId)) {
            executeOnTab(numTabId).catch(_ => {
              browser.tabs.query({ active: true }).then(tabs => {
                if (tabs && tabs[0]) browser.tabs.executeScript(tabs[0].id, { code: scriptCode, runAt: "document_end" });
                else browser.tabs.executeScript({ code: scriptCode, runAt: "document_end" });
              }).catch(e => console.error("[Remmi] EXECUTE_SCRIPT fallback failed", e));
            });
          } else {
            browser.tabs.query({ active: true }).then(tabs => {
              if (tabs && tabs[0]) {
                browser.tabs.executeScript(tabs[0].id, { code: scriptCode, runAt: "document_end" }).catch(e => {
                  console.error("[Remmi] EXECUTE_SCRIPT active tab failed", e);
                });
              } else {
                browser.tabs.executeScript({ code: scriptCode, runAt: "document_end" }).catch(e => {
                  console.error("[Remmi] EXECUTE_SCRIPT default tab failed", e);
                });
              }
            }).catch(e => {
              browser.tabs.executeScript({ code: scriptCode, runAt: "document_end" }).catch(err => {
                console.error("[Remmi] EXECUTE_SCRIPT direct failed", err);
              });
            });
          }
        }
...
async function getNativeDecision(details, cacheKey, requestId) {
  ...
  const promise = (async () => {
    ...
    const previousTabUrl = (details.tabId >= 0 && TAB_CURRENT_URLS.has(details.tabId)) ? TAB_CURRENT_URLS.get(details.tabId) : "";
    const sourceUrl = details.documentUrl || details.originUrl || previousTabUrl || "";
  ...
  async function(details) {
    ...
    try {
      ...
      if (shouldCancel) {
        ...
        if (resType === "main_frame" && details.tabId >= 0) {
          TAB_CURRENT_URLS.set(details.tabId, url);
        }
