...
/**
 * Argon2id and AES-256-GCM Password Crypto Engine for Remmi.
...
object PasswordCryptoEngine {
  ...
  fun evaluatePasswordStrength(password: CharArray): MasterPasswordStrength {
    ...
    val feedback = when {
      ...
      score >= 80 -> "Strong Master Password"
  ...
  fun evaluatePinStrength(pin: CharArray): Pair<Boolean, String> {
    if (pin.size < 8) {
      return Pair(false, "PIN must be at least 8 digits")
    }
    if (pin.size > 12) {
    ...
    // Check repeated chunk patterns (e.g. 12121212, 12312312, 12341234)
    for (period in 1..4) {
      if (pin.size >= period * 2) {
        var isPeriodic = true
        for (i in period until pin.size) {
          if (pin[i] != pin[i % period]) {
            isPeriodic = false
            break
          }
        }
        if (isPeriodic) {
          return Pair(false, "PIN contains a repetitive pattern. Use a diverse sequence.")
        }
      }
    }

    return Pair(true, "Strong PIN")
  }
  ...
  // --- 5. Canonical Exact-Origin and Site Hash for Autofill Matching ---
  /**
   * Canonicalize an exact origin from a URI string.
   * Returns "scheme://host[:port]" with:
   * - scheme strictly retained (http, https)
   * - host normalized with IDN to ASCII, lowercase, IPv6 brackets preserved, trailing dots stripped
   * - "www." is NOT stripped (distinct origin)
   * - Default ports (443 for https, 80 for http) omitted; non-default ports explicitly retained
   * Returns null if the URL is invalid or has an unsupported scheme.
   */
  fun canonicalizeOrigin(rawUrl: String?): String? {
    if (rawUrl.isNullOrBlank()) return null
    val trimmed = rawUrl.trim()
    val urlWithScheme = if (!trimmed.contains("://")) {
      "https://$trimmed"
    } else {
      trimmed
    }
    val uri = try {
      java.net.URI(urlWithScheme)
    } catch (_: Exception) {
      try {
        val parsed = android.net.Uri.parse(urlWithScheme)
        java.net.URI(parsed.scheme, parsed.userInfo, parsed.host, parsed.port, parsed.path, parsed.query, parsed.fragment)
      } catch (_: Exception) {
        null
      }
    } ?: return null
    ...
    val normalizedHost = if (rawHost.startsWith("[") && rawHost.endsWith("]")) {
      "[" + rawHost.substring(1, rawHost.length - 1).lowercase(java.util.Locale.US) + "]"
    } else {
      val hostWithoutDot = rawHost.trimEnd('.')
      if (hostWithoutDot.isBlank()) return null
      try {
        java.net.IDN.toASCII(hostWithoutDot).lowercase(java.util.Locale.US)
      } catch (_: Exception) {
        hostWithoutDot.lowercase(java.util.Locale.US)
      }
    }
  ...
  fun hashSiteUrl(url: String): String {
    val canonicalOrigin = canonicalizeOrigin(url) ?: url.trim().lowercase(java.util.Locale.US)
    val md = MessageDigest.getInstance("SHA-256")
    val digest = md.digest(canonicalOrigin.toByteArray(StandardCharsets.UTF_8))
    return digest.joinToString("") { "%02x".format(it) }
  }

  fun extractCanonicalHost(rawUrl: String): String {
    val canonical = canonicalizeOrigin(rawUrl) ?: rawUrl.trim()
    return try {
      val uri = java.net.URI(canonical)
